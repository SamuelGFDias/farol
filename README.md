...
**Status: núcleo funcional, sandboxed.** Seis features completas (core Rust + iced, protocolo de plugin JSON-RPC provado por quatro plugins de referência reais — `git-local`, `uptime-kuma`, `openfortivpn-vpn` e `docker-containers` — uma suíte de testes automatizada incluindo harness de smoke do binário real, e isolamento de processo real via `bwrap`: rede negada por padrão, filesystem restrito ao necessário, `exec` mediado). Ainda sem registry e sem empacotamento (fases 4–5 do roadmap abaixo). As decisões de arquitetura estão formalizadas numa constitution versionada em [`.specify/memory/constitution.md`](.specify/memory/constitution.md), e o projeto adota spec-driven development (spec-kit).
