...
- Q: A rota `/metrics` do Uptime Kuma exige autenticação até para o caso feliz desta feature, ou é opcional? → A: Sim, exige na prática — não é opcional. O Uptime Kuma protege `/metrics` com HTTP Basic Auth: a credencial é uma API Key gerada no próprio Uptime Kuma (usada como senha, com usuário vazio/qualquer valor) ou, na ausência de qualquer API Key, o usuário/senha da conta do Uptime Kuma — assim que a primeira API Key é criada, a autenticação por usuário/senha é desativada permanentemente. Esta feature MUST tratar autenticação como requisito desde o caso feliz, e a credencial MUST vir do keyring do sistema (Princípio IV da constitution), nunca de um arquivo de configuração em texto plano gerenciado pelo plugin — diferente do padrão usado para a URL base (FR-007/FR-008, que continua vindo de arquivo). Fonte: wiki oficial do projeto (Prometheus-Integration.md) e PR #101 do repositório `louislam/uptime-kuma`.
  ...
  > **Nota de correção (2026-09-01)**: o mecanismo concreto de armazenamento da credencial descrito acima ("keyring do sistema") foi revisado — ver FR-005, FR-019 e Princípio IV da constitution (versão 1.0.0) para o mecanismo atual (armazenamento dedicado gerenciado exclusivamente pelo core, não keyring do SO).
...
- A instância Uptime Kuma exige autenticação para expor `/metrics` — não é opcional, nem no caso feliz (ver FR-019). A credencial (API Key ou usuário/senha) MUST vir de um armazenamento dedicado gerenciado exclusivamente pelo core, nunca de um arquivo de configuração em texto plano gerenciado pelo plugin nem lido/escrito diretamente pelo plugin, por força do Princípio IV da constitution.
...
- **FR-005**: No handshake, o plugin MUST declarar quais variáveis de configuração precisa para funcionar — incluindo o host/endpoint da instância Uptime Kuma e a credencial de autenticação usada contra `/metrics` (ver FR-019) — através de um campo próprio do handshake, irmão do manifesto de capacidades, listando cada variável necessária com um nome estável, uma descrição legível e um indicador de se o valor é secreto. Nenhuma dessas variáveis MUST estar hardcoded no código do plugin; o valor de cada uma vem sempre resolvido pelo core (ver FR-008, FR-019), nunca lido pelo próprio plugin de um arquivo ou armazenamento que ele gerencie. Separadamente, e apenas quando o host já estiver resolvido a partir dessa configuração, o manifesto de capacidades MUST declarar a capacidade de rede correspondente (`kind: "network"`, com o host e, quando conhecida, a porta) — conforme o Princípio IV da constitution ("rede por allowlist de host declarada"); a declaração de capacidade de rede não carrega mais, ela própria, nenhum dado de credencial. Para acomodar esse metadado por capacidade (host/porta), o protocolo MUST evoluir de capacidades como lista de strings simples para uma lista de capacidades estruturadas, cada uma identificada por um `kind` (ex.: `exec`, `network`) e podendo carregar campos adicionais específicos daquele `kind` — esta especificação define esse rumo comportamental; o desenho exato dos dois schemas (o de capacidades estruturadas e o do campo que lista as variáveis de configuração necessárias, incluindo compatibilidade retroativa com a capacidade `exec` já existente) é decisão da etapa de planejamento técnico (`/speckit-plan`) desta feature, não desta especificação.
...
- **Manifesto de Capacidades**: conjunto de capacidades estruturadas que o plugin declara, incluindo, nesta feature, uma capacidade de rede referente ao host (e, quando conhecida, a porta) da instância Uptime Kuma já resolvida (FR-005) — declarada apenas quando esse host já estiver disponível. Apenas declarativo — sem enforcement nesta feature. A credencial de autenticação usada contra `/metrics` (FR-019) não faz parte deste manifesto — é declarada separadamente, junto com o host/endpoint em si, no campo de variáveis de configuração necessárias (ver "Configuração do Plugin" abaixo). O desenho exato do schema de capacidade estruturada é decisão da etapa de planejamento técnico (`/speckit-plan`).
...
- Esta é a primeira feature do projeto que exercita a parte "segredos" do Princípio IV da constitution — declaração de credencial pelo plugin e resolução do valor por um armazenamento dedicado gerenciado exclusivamente pelo core (FR-019) — o plugin `git-local` da feature 001 não precisou de nenhuma credencial.
